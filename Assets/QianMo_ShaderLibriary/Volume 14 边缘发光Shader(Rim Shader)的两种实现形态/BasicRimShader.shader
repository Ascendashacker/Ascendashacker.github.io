	...
	SubShader
	{
		...
		Pass
		{
			...
			CGPROGRAM
				...
				{
					//像素位置 || Pixel position
					float4 pos : SV_POSITION;   //SV_:表示系统值
					//一级纹理坐标 || Primary texture coordinates
					float4 texcoord : TEXCOORD0;
					//法线向量坐标 || Normal vector coordinates
					float3 normal : NORMAL;
					...
					LIGHTING_COORDS(3,4)  //这里的3和4指明变量的储存位置，这个宏大的定义会根据光源类型，有误cookie发生变化。  存储位置是什么？有什么作用？？
				};
				...
				fixed4 frag(VertexOutput i) : COLOR
				{
					...
					float3 finalColor = Diffuse * (tex2D(_TextureDiffuse,TRANSFORM_TEX(i.texcoord.rg, _TextureDiffuse)).rgb*_MainColor.rgb) + Emissive; //TRANSFORM_TEX:TRANSFORM_TEX(i.texcoord.rg, _TextureDiffuse)等同于i.texcoord.rg *_TextureDiffuse_ST.xy+_TextureDiffuse_ST.zw   //http://www.cnblogs.com/wantnon/p/5045124.html
