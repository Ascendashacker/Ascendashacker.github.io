Shader "浅墨Shader编程/Volume6/30.凹凸纹理+颜色可调+边缘光照"
{
	...
	SubShader 
	{
		...
		//【2】输入结构  
		struct Input   //系统固定命名结构，（是否跟表明着色器有关）
		{
			//主纹理的uv值
			float2 uv_MainTex; //uv_贴图名称：表示该贴图的uv值
			//凹凸纹理的uv值
			float2 uv_BumpMap;
			//当前坐标的视角方向
			float3 viewDir;  //系统内置变量
		};
		...
		void surf (Input IN, inout SurfaceOutput o) 
		{
			...
			//从_RimColor参数获取自发光颜色
			half rim = 1.0 - saturate(dot (normalize(IN.viewDir), o.Normal)); //saturate将值限制在0-1
			o.Emission = _RimColor.rgb * pow(rim, _RimPower);// rim *_RimPower;// 
		}
