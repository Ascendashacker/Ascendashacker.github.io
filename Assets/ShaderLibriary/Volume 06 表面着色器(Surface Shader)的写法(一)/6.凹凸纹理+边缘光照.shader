Shader "浅墨Shader编程/Volume6/29.凹凸纹理+边缘光照"
{
	...
	SubShader 
	{
		...
		void surf (Input IN, inout SurfaceOutput o) 
		{
			...
			//从_RimColor参数获取自发光颜色
			half rim = 1.0 - saturate(dot (normalize(IN.viewDir), o.Normal)); //saturate:如果 x 小于0，返回 0；如果 x大于 1，返回1；否则，返回x。normalize：归一化。dot：点积。法线方向是指向物体还是反向？
			o.Emission = _RimColor.rgb * pow (rim, _RimPower); //pow:次方
		}
