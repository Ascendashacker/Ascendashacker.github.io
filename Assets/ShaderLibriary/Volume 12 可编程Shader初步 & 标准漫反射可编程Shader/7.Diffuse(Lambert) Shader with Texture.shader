	...
	SubShader
		{
			...
			Pass
			{
				...
				//顶点着色器输出结构
				struct v2f  //v2f:表示vertex to flagment,字面命名习惯，从顶点着色器到片元着色器
				{
					float4 positon : SV_POSITION;//像素位置  ：和纹理坐标有什么区别？
					float3 normal : NORMAL;//法线向量坐标
					float2 texcoord : TEXCOORD0;//纹理坐标
				};

				//变量声明
				float4 _LightColor0;  //内置变量：灯光的颜色
				float4 _Color;
				sampler2D _MainTex;
				...
				v2f vert(appdata input)
				{
					...
					//获取顶点在世界空间中的法线向量坐标
					output.normal = mul(float4(input.normal, 0.0), unity_WorldToObject).xyz;  //新的API为：
					//输出的纹理坐标也就是输入的纹理坐标
