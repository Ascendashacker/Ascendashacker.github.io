	...
	SubShader
		{
			...
			Pass
			{
				...
				//变量声明
				float4 _LightColor0;  //内置变量：表示光照的颜色。放在哪个头文件里面
				float4 _Color;
				...
				fixed4 frag(v2f input) : COLOR
				{
					...
					//获取入射光线的值与方向
					float3 lightDirection = normalize(_WorldSpaceLightPos0.xyz); //_WorldSpaceLightPos0:内置变量，表示世界空间坐标系下灯光的位置
					...
					//【3】合并漫反射颜色值与环境光颜色值
					float4 DiffuseAmbient = float4(diffuse, 1.0) + UNITY_LIGHTMODEL_AMBIENT; //UNITY_LIGHTMODEL_AMBIENT:环境光颜色，rgba
